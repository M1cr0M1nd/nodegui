...
Napi::Object QListWidgetWrap::init(Napi::Env env, Napi::Object exports) {
  ...
  QOBJECT_REGISTER_WRAPPER(QListWidget, QListWidgetWrap);
  return exports;
}

QListWidget* QListWidgetWrap::getInternalInstance() { return this->instance; }
...
QListWidgetWrap::QListWidgetWrap(const Napi::CallbackInfo& info)
  ...
  if (argCount == 0) {
    ...
    if (info[0].IsExternal()) {
      ...
      this->instance = info[0].As<Napi::External<QListWidget>>().Data();
    } else {
    ...
    Napi::TypeError::New(
        env,
        "NodeGui: QListWidgetWrap: Wrong number of arguments to constructor")
