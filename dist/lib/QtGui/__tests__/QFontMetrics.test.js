...
const QFont_1 = require("../QFont");
const QFontMetrics_1 = require("../QFontMetrics");
describe('QFontMetrics', () => {
    // Helvetica is Qt's default sans-serif font
    const qfont = new QFont_1.QFont('Helvetica');
    it('initialize with QFont', () => {
        const metrics = new QFontMetrics_1.QFontMetrics(qfont);
    ...
    it('size', () => {
        ...
        const sizeDefault = metrics.size(QtEnums_1.TextFlag.None, 'abc');
        expect(sizeDefault.height()).toBeGreaterThan(0);
        expect(sizeDefault.width()).toBeGreaterThan(0);
        const sizeFlag1 = metrics.size(QtEnums_1.TextFlag.TextExpandTabs, '\tabc', 1);
        const sizeFlag10 = metrics.size(QtEnums_1.TextFlag.TextExpandTabs, '\tabc', 10);
    ...
    it('swap', () => {
        const metrics = new QFontMetrics_1.QFontMetrics(qfont);
        const metrics2 = new QFontMetrics_1.QFontMetrics(qfont);
